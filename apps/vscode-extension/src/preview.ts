...
<\/script>
  ...
  private constructor(
    ...
    this.panel.webview.onDidReceiveMessage(
      async (message: {
        ...
        language?: string;
        instruction?: string;
        // v1.7.0 — selection AI bridge (mountSelectionToolbar →
        // buildAiMenu in @mdcore/editor). The webview correlates
        ...
        switch (message.type) {
          ...
          case "selectionAi": {
            // v1.7.0 — selection-only AI flow (separate from
            // handleAiAction which rewrites the whole document).
            // The webview sends snippet markdown + selection_*
            // action and we hit /api/ai with selection scope. The
            // result is posted back with the originating requestId
            // so the webview can correlate and insert it into
            // exactly one selection range.
            await this.handleSelectionAi(
              String(message.requestId || ""),
              String(message.action || ""),
              String(message.markdown || ""),
              typeof message.language === "string" ? message.language : undefined,
              typeof message.instruction === "string" ? message.instruction : undefined
            );
            break;
          }
          case "syncToLocal": {
  ...
  /**
   * v1.7.0 — handler for the floating selection toolbar's AI menu
   * (mountSelectionToolbar / buildAiMenu in @mdcore/editor). The
   * webview ships only the SELECTED snippet plus the action + opts;
   * we route to /api/ai and post the result back as `selectionAiResult`
   * with the originating requestId so the webview can correlate and
   * insert the result into the saved selection range (not the whole
   * doc). Separate from handleAiAction which still drives the bigger
   * AI panel that rewrites the whole document.
   */
  private async handleSelectionAi(
    requestId: string,
    action: string,
    snippet: string,
    language?: string,
    ...
    const reply = (payload: { result?: string; error?: string }): void => {
      this.panel.webview.postMessage({ type: "selectionAiResult", requestId, ...payload });
    };
    if (!action || !snippet.trim()) {
      reply({ error: "Empty selection." });
      return;
    }
    if (snippet.length > 8000) {
      reply({ error: "Selection too long (max 8k chars)." });
      return;
    }
    const auth = PreviewPanel.authManagerRef;
    const loggedIn = await auth?.isLoggedIn();
    if (!auth || !loggedIn) {
      reply({ error: "Sign in to use AI features." });
      const choice = await vscode.window.showWarningMessage(
        "Sign in to memory.wiki to use AI features.",
        "Sign in"
      );
    ...
    try {
      ...
      const body: Record<string, string> = { action, markdown: snippet };
      if (language) body.language = language;
      if (instruction) body.instruction = instruction;
      ...
      const out = (data.result || "").replace(/^(EDIT:|ANSWER:)\s*/, "").trim();
...
<body${this.isCloudPreview ? ' class="live-mode"' : ""} data-read-only="${this.isCloudPreview ? "true" : "false"}">
  <!-- Loading state -->
  ...
  <!-- Global toolbar: view mode toggle only. Wordmark removed per
       founder ask 2026-06-01 — preview tab already shows the brand
       blob in its title bar, so a second wordmark in the body was
       redundant chrome. -->
  <div id="toolbar">
  ...
  <div id="editor-wrapper">
    ...
    <div id="live-pane">
      ...
      <div id="live-formatting-toolbar" class="pane-toolbar">
        ...
        <button data-action="h1" title="Heading 1: # text" data-preview="<span style='font-size:1.3em;font-weight:700'>Heading 1</span>"><span style="font-size:10px;font-weight:700">H1</span></button>
        <button data-action="h2" title="Heading 2: ## text" data-preview="<span style='font-size:1.1em;font-weight:600'>Heading 2</span>"><span style="font-size:10px;font-weight:700">H2</span></button>
        <button data-action="h3" title="Heading 3: ### text" data-preview="<span style='font-size:0.95em;font-weight:600'>Heading 3</span>"><span style="font-size:10px;font-weight:600">H3</span></button>
        <button data-action="h4" title="Heading 4: #### text"><span style="font-size:10px">H4</span></button>
        <button data-action="h5" title="Heading 5: ##### text"><span style="font-size:10px">H5</span></button>
        <button data-action="h6" title="Heading 6: ###### text"><span style="font-size:10px">H6</span></button>
        <button data-action="p" title="Paragraph: normal text" data-preview="<span style='font-size:0.85em'>Normal paragraph text</span>"><span style="font-size:10px">P</span></button>
        <span class="toolbar-divider"></span>
        <button data-action="bold" title="Bold (Cmd+B): **text**" data-preview="<b>Bold text</b>"><span style="font-weight:700;font-size:12px">B</span></button>
        <button data-action="italic" title="Italic (Cmd+I): *text*" data-preview="<i>Italic text</i>"><span style="font-style:italic;font-size:12px">I</span></button>
        <button data-action="strikethrough" title="Strikethrough: ~~text~~" data-preview="<s style='opacity:0.6'>Strikethrough</s>"><span style="text-decoration:line-through;font-size:12px">S</span></button>
        <button data-action="code" title="Inline code: \`code\`" data-preview="<code style='font-family:monospace;font-size:11px;background:rgba(255,255,255,0.1);padding:2px 6px;border-radius:3px'>inline code</code>"><span style="font-family:monospace;font-size:10px">&lt;/&gt;</span></button>
        <span class="toolbar-divider"></span>
        ...
        <span class="toolbar-divider"></span>
        <button data-action="blockquote" title="Blockquote: > text" data-preview="<div style='border-left:3px solid #fb923c;padding-left:8px;font-style:italic;opacity:0.8'>Quoted text</div>"><svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M3 3h4v4H5.5L4 10H3V3zm6 0h4v4h-1.5L10 10H9V3z"/></svg></button>
  ...
  <!-- v1.7.0: the floating selection toolbar is now built + positioned
       by @mdcore/editor's mountSelectionToolbar() helper (mounted in
       preview-tiptap.js). This static stub stays so any code path
       that reads getElementById('selection-toolbar') still finds an
       element, but it never paints — display:none. -->
