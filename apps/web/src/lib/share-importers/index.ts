...
export async function importShare(rawUrl: string): Promise<ShareImportResult> {
  ...
  switch (provider) {
    ...
    case "claude":
      throw new ShareImportError(
        "Claude shares can't be fetched server-side — Cloudflare blocks it. Use the Memory.Wiki bookmarklet on the chat page instead: https://memory.wiki/bookmarklet",
        { status: 501 }
      );
    case "gemini":
      throw new ShareImportError(
        "Gemini share-link import isn't available yet. For now, use the Memory.Wiki bookmarklet on the chat page: https://memory.wiki/bookmarklet",
        { status: 501 }
      );
  }
}
